...
const getCategoryEntityIdbyHandle = async (handle: string) => {
  ...
  return recursiveFindCollectionId(
    resp.body.data.site.categoryTree,
    handle,
  )
}
...
const getBigCommerceProductsWithCheckout = async (
  ...
  const resCheckout =
    await bigCommerceFetch<BigCommerceCheckoutOperation>({
...
export async function addToCart(
  ...
  const { productsByIdList, checkout, checkoutUrl } =
    await getBigCommerceProductsWithCheckout(
      bigCommerceCart.entityId,
      lines,
    )

  return bigCommerceToVercelCart(
    bigCommerceCart,
    productsByIdList,
    checkout,
    checkoutUrl,
...
export async function removeFromCart(
  ...
  let cartState: {
    status: number
    body: BigCommerceDeleteCartItemOperation
  }
  const removeCartItem = async (itemId: string) => {
    const res =
      await bigCommerceFetch<BigCommerceDeleteCartItemOperation>({
  ...
  const { productsByIdList, checkout, checkoutUrl } =
    await getBigCommerceProductsWithCheckout(cartId, lines)
...
export async function updateCart(
  ...
  let cartState:
    | { status: number; body: BigCommerceUpdateCartItemOperation }
    | undefined
  ...
  const updatedCart =
    cartState!.body.data.cart.updateCartLineItem.cart
  const { productsByIdList, checkout, checkoutUrl } =
    await getBigCommerceProductsWithCheckout(cartId, lines)
...
export async function getCollectionProducts({
  ...
  if (
    expectedCollectionBreakpoints[collection] ===
    "carousel_collection"
  ) {
    const res =
      await bigCommerceFetch<BigCommerceNewestProductsOperation>({
  ...
  if (
    ...
    const res =
      await bigCommerceFetch<BigCommerceFeaturedProductsOperation>({
  ...
  const res =
    await bigCommerceFetch<BigCommerceProductsCollectionOperation>({
...
export async function getCollections(): Promise<VercelCollection[]> {
  ...
  const collections = await Promise.all(
    collectionIdList.map(async (entityId) => {
      const res =
        await bigCommerceFetch<BigCommerceCollectionOperation>({
          query: getCategoryQuery,
          variables: {
            entityId,
          },
        })
      return bigCommerceToVercelCollection(
        res.body.data.site.category,
      )
    }),
  )
...
export async function getMenu(handle: string): Promise<VercelMenu[]> {
      ...
      return menuData
        .flatMap((item) => {
          ...
          if (menuType === "footer") {
            ...
            return vercelMenuItem.title === "Blog" ||
  ...
  if (handle === "next-js-frontend-header-menu") {
    ...
    return configureVercelMenu(
      res.body.data.site.categoryTree,
      true,
      "header",
    )
  }
...
export async function getPage(handle: string): Promise<VercelPage> {
  ...
  return bigCommerceToVercelPageContent(
    res.body.data.site.content.page,
  )
}
...
export async function getProductRecommendations(
  ...
  const res =
    await bigCommerceFetch<BigCommerceRecommendationsOperation>({
  ...
  const productList =
    res.body.data.site.product.relatedProducts.edges.map(
      (item) => item.node,
    )
...
export async function getProducts({
  ...
  const res =
    await bigCommerceFetch<BigCommerceSearchProductsOperation>({
